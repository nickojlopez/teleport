...
func (h *Handler) generateCommandSession(ctx context.Context, clt auth.ClientI, req *CommandRequest, clusterName string, owner string) (session.Session, error) {
	h.log.Infof("Generating new session for %s\n", clusterName)

	host, port, id, err := findByHost(ctx, clt, req.NodeID) //TODO(jakule): Add search by label
	if err != nil {
		return session.Session{}, err
	}
