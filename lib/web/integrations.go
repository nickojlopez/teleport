...
// integrationAllowedUpdateStatus contains a list of available status for an Integration when trying to update it.
var integrationAllowedUpdateStatus = []types.IntegrationSpecV1_IntegrationStatus{
	// Integration is not running because it was disabled.
	types.IntegrationSpecV1_INTEGRATION_STATUS_PAUSED,
	// Integration is running.
	types.IntegrationSpecV1_INTEGRATION_STATUS_RUNNING,
}
...
func (h *Handler) integrationsUpdate(w http.ResponseWriter, r *http.Request, p httprouter.Params, sctx *SessionContext, site reversetunnel.RemoteSite) (interface{}, error) {
	...
	if req.AWSOIDC != nil {
		if integration.GetSubKind() != types.IntegrationSubKindAWSOIDC {
			return nil, trace.BadParameter("cannot update aws-oidc fields for a %q integration", integration.GetSubKind())
		}
	...
	if req.Status != nil {
		// Status can only be updated to Paused or Running.
		// The Error status must come from the service and not from clients.
		if !slices.Contains(integrationAllowedUpdateStatus, req.IntegrationStatus) {
			return nil, trace.BadParameter("invalid status: must be either Paused or Running")
		}

		integration.SetStatusCode(req.IntegrationStatus)
	}
